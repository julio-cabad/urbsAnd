...
function UpdateTwoFields(dataArray, field, param, key_1, value_1, key_2, value_2) {
    ...
    let newArray = [...dataArray];
    newArray[elementsIndex] = {
        ...newArray[elementsIndex], [key_1]: value_1, [key_2]: value_2
    };
    return newArray;
}
...
function _homeTown(addressArray) {
    const arr = [];
    addressArray?.forEach(items => {
        cities.forEach(c => {
            const city = items.toUpperCase();
            if (c.name === city) {
                arr.push(items);
            }
        });
...
export {
    ...
    FilterTwo, duplicateArrayObjects, capitalizeFirstLetter, _homeTown, UpdateTwoFields
};
